...
<table border="1" style="color: black;">
  ...
  <tr>
    ...
    $s = "select name,address, mobile_no, email, mname, mcode, advance from machineries";
    $result = mysqli_query($conn,$s);
    $num = mysqli_num_rows($result);
    if ($num >0) {
       while ($row = mysqli_fetch_assoc($result) ) {
         echo "<tr><td>".$row["name"]."</td><td>".$row["address"]."</td><td>".$row["mobile_no"]."</td><td>".$row["email"]."</td><td>".$row["mname"]."</td><td>".$row["mcode"]."</td><td>".$row["advance"]."</td></tr>";
       }
    }
